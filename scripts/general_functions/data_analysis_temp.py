...
def find_overshoot(list_data_per):
    overshoot = []
    for data in list_data_per:
        overshoot.append(max(data)-1)
    return overshoot


def find_rise_time(list_data_per, list_data_time, rise_time_threshold=0.8):
    rise_time = []
    for data, t in zip(list_data_per, list_data_time):
        ind_time = next(i for i, v in enumerate(data) if v > rise_time_threshold)
...
def canalyze_centering_per(data_list, fig_title):
    ...
    list_target_radius_jacobian_per = data_abs_to_per(list_target_radius_jacobian, target_center=1)
    list_target_radius_potential_per = data_abs_to_per(list_target_radius_potential, target_center=1)
    ...
    # steady_state_error
    target_x_jacobian_steady_state_error = find_steady_state_error(list_target_x_jacobian_per)
    target_y_jacobian_steady_state_error = find_steady_state_error(list_target_y_jacobian_per)
    target_radius_jacobian_steady_state_error = find_steady_state_error(list_target_radius_jacobian_per)
    target_x_potential_steady_state_error = find_steady_state_error(list_target_x_potential_per)
    target_y_potential_steady_state_error = find_steady_state_error(list_target_y_potential_per)
    target_radius_potential_steady_state_error = find_steady_state_error(list_target_radius_potential_per)
    ...
    print('SSerror J_Y:', target_y_jacobian_steady_state_error)
    print('SSerror:', average_steady_state_error, variance_steady_state_error)

    # over_shoot
    target_x_jacobian_overshoot = find_overshoot(list_target_x_jacobian_per)
    target_y_jacobian_overshoot = find_overshoot(list_target_y_jacobian_per)
    target_x_potential_overshoot = find_overshoot(list_target_x_potential_per)
    target_y_potential_overshoot = find_overshoot(list_target_y_potential_per)
    average_overshoot = [calculate_list_average(target_x_jacobian_overshoot),
                         calculate_list_average(target_y_jacobian_overshoot),
                         calculate_list_average(target_x_potential_overshoot),
                         calculate_list_average(target_y_potential_overshoot)]
    variance_overshoot = [calculate_list_std(target_x_jacobian_overshoot),
                          calculate_list_std(target_y_jacobian_overshoot),
                          calculate_list_std(target_x_potential_overshoot),
                          calculate_list_std(target_y_potential_overshoot)]
    # No over shoot for target radius
    list_data_average.append(average_overshoot)
    list_data_variance.append(variance_overshoot)
    print('OS:', average_overshoot, variance_overshoot)
    ...
    labels = ['J_x', 'J_y', 'J_r', 'PF_x', 'PF_y', 'PF_r']
    labels_less = ['J_x', 'J_y', 'PF_x', 'PF_y']
    list_label = [labels, labels_less, labels_less, labels_less]
    fig, axes = plt.subplots(1, 4)
    fig.suptitle(fig_title, fontsize=16)
    for ax, label, average, variance in zip(axes, list_label, list_data_average, list_data_variance):
        ax.bar(label, average, yerr=variance,
                align='center', alpha=0.5, ecolor='black', capsize=10)
        for i, v in enumerate(average):
            ax.text(i, v, str(round(v, 2)), color='blue', va='center', fontweight='bold')
    ...
    # axes[1].bar(labels_less, np.array(average_overshoot) - 1, yerr=variance_overshoot, # to be determined?
    #             align='center', alpha=0.5, ecolor='black', capsize=10)
...
if __name__ == '__main__':
    ...
    canalyze_centering_per(saved_data_list, 'ic_1')
    canalyze_centering_num(saved_data_list)
    directory = os.getcwd() + '\\to_analyze\\task_1\\ic_2\\'
    saved_data_list = canalyze_centering_img(directory)
    canalyze_centering_per(saved_data_list, 'ic_2')
    canalyze_centering_num(saved_data_list)
    directory = os.getcwd() + '\\to_analyze\\task_1\\all\\'
    saved_data_list = canalyze_centering_img(directory)
    canalyze_centering_per(saved_data_list, 'all')
    canalyze_centering_num(saved_data_list)
    plt.show()
